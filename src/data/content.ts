...
const markdownFiles = import.meta.glob('../content/cycle-*/*/*/chapters/*.md', { query: '?raw', eager: true });
  ...
  for (const topic of subject.topics) {
    if (topic.type === 'sheet') {
      ...
      const matchingPath = Object.keys(markdownFiles).find(path => {
        ...
        return normalizedPath.includes(expectedPathSegment) && (normalizedPath.includes(slug) || normalizedPath.includes(topic.id));
      });

      if (matchingPath) {
        const rawContent = markdownFiles[matchingPath] as any;
        topic.content = (rawContent.default ?? rawContent) as string;
      } else {
